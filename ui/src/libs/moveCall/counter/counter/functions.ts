...
export function create(tx: Transaction) {
  return tx.moveCall({ target: `${PUBLISHED_AT}::counter::create`, arguments: [] });
}
...
export function setValue(tx: Transaction, args: SetValueArgs) {
  return tx.moveCall({
    target: `${PUBLISHED_AT}::counter::set_value`,
