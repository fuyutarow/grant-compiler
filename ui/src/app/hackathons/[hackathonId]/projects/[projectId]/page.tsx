  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 text-black">
      ...
      <main className="flex-grow flex flex-col items-center p-8 w-full">
        <Link href={`/hackathons/${hackathonId}/projects`} className="mb-4 text-blue-500 hover:underline self-start">
          &larr; Back to Projects
        </Link>
        <div className="w-full max-w-4xl bg-white shadow-lg rounded-lg p-6">
          <div className="flex flex-col md:flex-row items-center md:items-start">
            <img src={project.logo || "https://via.placeholder.com/150"} alt="Project Logo" className="w-32 h-32 object-cover rounded-full md:mr-6 mb-4 md:mb-0" />
            <div className="flex-grow">
              <h1 className="text-3xl font-bold mb-2">{project.title}</h1>
              <div className="flex flex-wrap mb-4">
                {project.tags.map((tag, index) => (
                  <span key={index} className="inline-block bg-blue-200 text-blue-800 text-xs px-2 py-1 rounded-full mr-2 mb-2">{tag}</span>
                ))}
              </div>
              ...
              <div className="flex flex-col mb-4">
                {project.links.map((link, index) => (
                  <a key={index} href={link.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline mb-2">{link.url}</a>
                ))}
              </div>
